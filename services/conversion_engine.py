...
class ConversionEngine:
    def __init__(self, redis_client: Any, session_timeout_seconds: int = 600):
    ...
    def _active_session_key(self, track_id: str) -> str:
        return f'funnel:active_session:{track_id}'

    def _session_hash_key(self, session_id: str) -> str:
        return f'funnel:session:{session_id}'
    ...
    def _ensure_active_session(self, track_id: str, timestamp: float) -> str:
        ...
        self.redis.sadd('funnel:entered_store', session_id)
        self.redis.zadd('funnel:entry_timestamps', {session_id: timestamp})
        return session_id
    ...
    def _close_session(self, track_id: str) -> None:
        ...
        if session_id:
            ...
            self.redis.set(f'funnel:last_exit:{track_id}', time.time())
    ...
    def process_customer_events(self, events: List[Dict[str, Any]]) -> None:
        for event in events:
            ...
            if event_type == 'entry':
                ...
                if not self.redis.sismember('funnel:browsed_gt_2min', session_id):
                    self.redis.sadd('funnel:browsed_gt_2min', session_id)
                ...
                session_id = self._ensure_active_session(customer_id, timestamp)
                if not self.redis.sismember('funnel:reached_checkout_zone', session_id):
                    self.redis.sadd('funnel:reached_checkout_zone', session_id)
